            ...
            foreach (var producto in productosStockBajo)
            {
                ...
                if (!alertaExiste)
                {
                    var alerta = new Alerta
                    {
                        ...
                        Mensaje = $"Stock bajo de {producto.NombreProducto}: {producto.StockActual} unidades",
                        RolDestino = "Administrador",
                        Prioridad = producto.StockActual == 0 ? "Crítica" : "Alta",
        ...
        public async Task GenerarAlertasCitasAsync()
        {
            ...
            foreach (var cita in citasProximas)
            {
                ...
                var alertaPaciente = new Alerta
                {
                    ...
                    UsuarioDestinoId = cita.PacienteId,
                    Prioridad = "Media",
                ...
                {
                    TipoAlerta = "CitaProxima",
                    Mensaje = $"Cita próxima: {cita.Paciente.NombreCompleto} el {cita.FechaCita:dd/MM/yyyy HH:mm}",
